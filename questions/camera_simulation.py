...
file_data_path= "/Users/guadalupecantera/Desktop/cs1430/homework3_cameracalibration-gcantera5/questions/images/bunny.png"
...
def calculate_camera_matrix(tx, ty, tz, alpha, beta, gamma, fx, fy, skew, u, v):
    ...
    neg_sin_alpha = -np.sin(alpha)
    sin_alpha = np.sin(alpha)
    cos_beta = np.cos(beta)
    cos_gamma = np.cos(gamma)
    neg_sin_gamma = -np.sin(gamma)
    sin_gamma = np.sin(gamma)
    neg_sin_beta = -np.sin(beta)
    sin_beta= np.sin(beta)   
    rotate_X_axis = np.array([[1, 0, 0],[0, cos_alpha, neg_sin_alpha],[0, sin_alpha, cos_alpha]])
    rotate_Y_axis = np.array([[cos_beta, 0, sin_beta],[0, 1, 0],[neg_sin_beta, 0, cos_beta]])
    rotate_Z_axis = np.array([[cos_gamma, neg_sin_gamma, 0],[sin_gamma, cos_gamma, 0] ,[0, 0, 1]])
    initial_extrinsic_matrix_to_replace = rotate_X_axis * rotate_Y_axis* rotate_Z_axis 
    initial_extrinsic_matrix_to_replace = np.append(initial_extrinsic_matrix_to_replace, [[tx],[ty],[tz]], axis=1)
    initial_intrinsic_matrix_to_replace = [[fx, skew, u],[0, fy, v],[0, 0, 1]]
